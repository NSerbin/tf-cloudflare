## Portainer App
resource "cloudflare_access_application" "portainer_app" {
...
## Portainer Policy
resource "cloudflare_access_policy" "portainer_policy_default" {
  application_id = cloudflare_access_application.portainer_app.id
  zone_id        = cloudflare_zone.nserbin_website_zone.id
  name           = "Default Policy"
  precedence     = "1"
  decision       = "allow"
