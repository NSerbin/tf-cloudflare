resource "cloudflare_access_application" "homepage_app" {
...
resource "cloudflare_access_policy" "homepage_policy_default" {
  application_id = cloudflare_access_application.homepage_app.id
  zone_id        = cloudflare_zone.nserbin_website_zone.id
  name           = "Default Policy"
  precedence     = "1"
  decision       = "allow"
