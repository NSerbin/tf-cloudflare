## IT-Tools App
resource "cloudflare_zero_trust_access_application" "it-tools_app" {
  zone_id                    = cloudflare_zone.nserbin_website_zone.id
  name                       = var.it-tools["name"]
  domain                     = var.it-tools["domain"]
  ...
  destinations = [
    {
      type = "public"
      uri  = var.it-tools["domain"]
    }
  ]
  #logo_url = var.it-tools["logo_url"]
}

## Record for it-tools
resource "cloudflare_dns_record" "it-tools_record" {
  zone_id = cloudflare_zone.nserbin_website_zone.id
  name    = var.it-tools["domain"]
